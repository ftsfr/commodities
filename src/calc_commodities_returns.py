"""
Calculate commodity returns using various data sources.
"""
...
def wide_to_long_returns(df, list_of_return_ticker):
    """Convert wide-format return DataFrame to long-format (ds, unique_id, y)."""
...
def calc_return_manual(df):
    """Calculate manual returns from futures data."""
    df = df[df["Contract"] <= 4]
...
def generate_replication_gsci(data_dir=DATA_DIR):
    """Generate replication using GSCI indices."""
    df_return1 = load_futures_data.load_gsci_data(data_dir=data_dir)
    hkm_df = extract_hkm_cmdty.extract_hkm_cmdty(data_dir=data_dir)
...
def calc_lme_monthly_1mprice(lme_df, metal_map, date_col="index", price_func=None):
    """Calculate LME monthly 1-month prices."""
    df = lme_df.copy()
...
def compute_second_contract_return(commodity_futures_df, date_col="index"):
    """Compute returns from second-nearest contracts."""
...
def generate_replication_future_ticker(data_dir=DATA_DIR):
    """Generate replication using futures tickers."""
    hkm_df = extract_hkm_cmdty.extract_hkm_cmdty(data_dir=data_dir)
...
def load_commodities_returns(data_dir=DATA_DIR):
    """Load commodity returns (GSCI-based replication)."""
...
if __name__ == "__main__":
    ...
    print("Replication outputs saved:")
